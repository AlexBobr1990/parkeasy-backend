...
async function main() {
  ...
  for (const sign of allSigns) {
    ...
    blk.coords.push([coord.lng, coord.lat]); // GeoJSON: [lng, lat]
    blk.rules.push(...parsed.rules);
    blk.zoneTypes.push(parsed.zoneType);
    parsedOk++;
  }
  ...
  for (const [key, blk] of blocks) {
    ...
    // Основной тип зоны (приоритет: hydrant > no_standing > asp > no_parking)
    const typePri = { hydrant: 4, no_standing: 3, asp: 2, no_parking: 1, school: 1 };
    let zoneType = 'no_parking';
    for (const zt of blk.zoneTypes) {
      if ((typePri[zt] || 0) > (typePri[zoneType] || 0)) zoneType = zt;
    }
    // Если есть BROOM, всегда asp (даже если рядом NO PARKING на том же блоке)
    if (blk.zoneTypes.includes('asp')) zoneType = 'asp';
    ...
    const srcParts = [blk.borough, blk.streetName, blk.fromStreet, blk.toStreet, blk.compassSide];
