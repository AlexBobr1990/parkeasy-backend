...
const OFFSET_MIN = 3;  // Минимум 3м от центра (защита от аномалий)
const OFFSET_MAX = 15; // Максимум 15м (защита от ошибок координат)
const OFFSET_DEFAULT = 8; // Дефолт если не удалось измерить
...
// Snap zone to OSM way: align direction, keep length, offset to REAL curb distance
function snapAndOffset(zoneCoords, wayCoords, side) {
  ...
  // Реальное расстояние от знака до центра дороги (в метрах)
  const latCenter = zoneCenter[1];
  const mPerDegLng = 111320 * Math.cos(latCenter * Math.PI / 180);
  const dLng = (zoneCenter[0] - proj[0]) * mPerDegLng;
  const dLat = (zoneCenter[1] - proj[1]) * mPerDeg;
  const realDistMeters = Math.sqrt(dLng * dLng + dLat * dLat);

  // Clamp: min 3m, max 15m
  const offsetMeters = Math.max(OFFSET_MIN, Math.min(OFFSET_MAX, realDistMeters || OFFSET_DEFAULT));

  return offsetLine(newCoords, nearest.bearing, side, offsetMeters);
}
...
async function main() {
  ...
  console.log(`   Offset: реальное расстояние от центра дороги (${OFFSET_MIN}-${OFFSET_MAX}м)`);
