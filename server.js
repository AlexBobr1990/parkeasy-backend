...
const userSchema = new mongoose.Schema({
  ...
  experience: { type: Number, default: 0 }, // XP для расчёта уровня
  totalPointsEarned: { type: Number, default: 0 }, // Всего заработано баллов
...
// Настройки геймификации (редактируется в админке)
const gameSettingsSchema = new mongoose.Schema({
  ...
  dailyTaskReward: { type: Number, default: 10 },
...
const userStatusSchema = new mongoose.Schema({
  ...
  status: { type: String, enum: ['none', 'going_home', 'looking_parking', 'can_share'], default: 'none' },
  updatedAt: { type: Date, default: Date.now }
});
...
const userDailyProgressSchema = new mongoose.Schema({
  ...
  allTasksBonus: { type: Boolean, default: false },
  loginRecorded: { type: Boolean, default: false }
});
...
const seasonalEventSchema = new mongoose.Schema({
  ...
  description: { en: String, ru: String, es: String, uk: String },
  icon: String,
  pointsMultiplier: { type: Number, default: 1.5 },
  startDate: Date,
  endDate: Date,
  isActive: { type: Boolean, default: true },
  specialTasks: [{
    ...
    name: { en: String, ru: String, es: String, uk: String },
    type: String, targetValue: Number, reward: Number
  }],
  specialAchievement: {
    code: String, icon: String,
    name: { en: String, ru: String, es: String, uk: String },
...
const leaderboardSchema = new mongoose.Schema({
  ...
  rankings: [{
    userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
    name: String, avatar: String, level: Number, levelIcon: String,
    score: Number, parkingsGiven: Number, rank: Number
  }],
  updatedAt: { type: Date, default: Date.now }
});
leaderboardSchema.index({ period: 1, city: 1 }, { unique: true });
...
const userCitySchema = new mongoose.Schema({
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', unique: true },
  city: String,
  cityCounts: mongoose.Schema.Types.Mixed,
  updatedAt: { type: Date, default: Date.now }
});
...
app.post('/api/parkings/:id/confirm-meet', async (req, res) => {
  try {
    ...
    await updateDailyTaskProgress(parking.ownerId, 'give_parking');
    await updateDailyTaskProgress(parking.bookedBy, 'receive_parking');
    
    // Проверяем достижения
    await checkAchievements(parking.ownerId, 'parking_completed');
    await checkAchievements(parking.bookedBy, 'parking_completed');
...
const calculateUserLevel = async (user) => {
  const settings = await GameSettings.findOne({ key: 'main' });
  if (!settings || !settings.levels || settings.levels.length === 0) {
    return { level: 1, name: { en: 'Newbie', ru: 'Новичок' }, icon: '🚗', progress: 0 };
  }
  
  const totalPoints = (user.balance || 0) + (user.totalPointsEarned || 0);
  const parkingsGiven = user.parkingsGiven || 0;
  
  const sortedLevels = [...settings.levels].sort((a, b) => b.minPoints - a.minPoints);
  
  for (const lvl of sortedLevels) {
    if (totalPoints >= lvl.minPoints && parkingsGiven >= lvl.minParkingsGiven) {
      const nextLevel = settings.levels.find(l => l.level === lvl.level + 1);
      let progress = 100;
      if (nextLevel) {
        const pointsProgress = Math.min((totalPoints - lvl.minPoints) / (nextLevel.minPoints - lvl.minPoints), 1);
        const parkingsProgress = Math.min((parkingsGiven - lvl.minParkingsGiven) / (nextLevel.minParkingsGiven - lvl.minParkingsGiven), 1);
        progress = Math.floor((pointsProgress + parkingsProgress) / 2 * 100);
      }
      return { level: lvl.level, name: lvl.name, icon: lvl.icon, progress, minPoints: lvl.minPoints, minParkingsGiven: lvl.minParkingsGiven };
    }
  }
  
  const firstLevel = settings.levels.find(l => l.level === 1) || { level: 1, name: { en: 'Newbie', ru: 'Новичок' }, icon: '🚗' };
  return { level: 1, name: firstLevel.name, icon: firstLevel.icon, progress: 0 };
};
...
const updateUserStreak = async (userId) => {
  ...
  if (!streak) {
    streak = new UserStreak({ userId, currentStreak: 1, lastActiveDate: today });
    await streak.save();
    return streak;
  }
...
const updateDailyTaskProgress = async (userId, taskType, increment = 1) => {
  const today = getTodayDate();
  let progress = await UserDailyProgress.findOne({ userId, date: today });
  
  if (!progress) {
    const activeTasks = await DailyTaskConfig.find({ isActive: true });
    progress = new UserDailyProgress({
      userId, date: today,
      tasks: activeTasks.map(t => ({ taskCode: t.code, currentValue: 0, completed: false, rewardClaimed: false }))
    });
  }
  
  const taskConfig = await DailyTaskConfig.findOne({ type: taskType, isActive: true });
  if (!taskConfig) { await progress.save(); return progress; }
  
  const taskProgress = progress.tasks.find(t => t.taskCode === taskConfig.code);
  if (!taskProgress || taskProgress.completed) { await progress.save(); return progress; }
  
  taskProgress.currentValue += increment;
  if (taskProgress.currentValue >= taskConfig.targetValue) {
...
const checkAchievements = async (userId, triggerType = null) => {
  const user = await User.findById(userId);
  if (!user) return [];
  ...
  for (const config of configs) {
    const hasAchievement = user.achievements?.some(a => a.id === config.code);
    ...
    switch (condType) {
      case 'parkings_given':
        unlocked = (user.parkingsGiven || 0) >= condValue;
        break;
      case 'streak':
        const streak = await UserStreak.findOne({ userId });
        unlocked = (streak?.currentStreak || 0) >= condValue;
        break;
      case 'rating':
        unlocked = user.rating >= condValue && user.ratingCount >= 10;
        break;
    }
    
    if (unlocked) {
      if (!user.achievements) user.achievements = [];
      user.achievements.push({ id: config.code, unlockedAt: new Date() });
      
      if (config.reward > 0) {
        user.balance += config.reward;
        await new Transaction({ userId, type: 'bonus', amount: config.reward, description: `Achievement: ${config.code}` }).save();
      }
      await user.save();
...
app.get('/api/users/:id/status', async (req, res) => {
  try {
    const status = await UserStatus.findOne({ userId: req.params.id });
    res.json({ status: status?.status || 'none', updatedAt: status?.updatedAt });
  } catch (error) {
...
app.get('/api/users/:id/daily-tasks', async (req, res) => {
  try {
    ...
    if (!progress.loginRecorded) {
      progress.loginRecorded = true;
      await updateDailyTaskProgress(userId, 'login');
      progress = await UserDailyProgress.findOne({ userId, date: today });
    }
    ...
    const tasks = progress.tasks.map(t => {
      const config = taskConfigs.find(c => c.code === t.taskCode);
      return {
        code: t.taskCode, icon: config?.icon || '📋', name: config?.name || {},
        type: config?.type, currentValue: t.currentValue, targetValue: config?.targetValue || 1,
        completed: t.completed, rewardClaimed: t.rewardClaimed, reward: config?.reward || 10
      };
    });
    ...
    res.json({
      ...
      allTasksBonusClaimed: progress.allTasksBonus,
...
app.post('/api/users/:id/daily-tasks/:taskCode/claim', async (req, res) => {
  try {
    ...
    const task = progress.tasks.find(t => t.taskCode === taskCode);
    if (!task || !task.completed || task.rewardClaimed) {
      return res.status(400).json({ success: false, message: 'Cannot claim' });
    }
    
    const taskConfig = await DailyTaskConfig.findOne({ code: taskCode });
    const reward = taskConfig?.reward || 10;
    ...
    await new Transaction({ userId, type: 'bonus', amount: reward, description: `Daily task: ${taskCode}` }).save();
...
app.post('/api/users/:id/daily-tasks/claim-all-bonus', async (req, res) => {
  try {
    ...
    const progress = await UserDailyProgress.findOne({ userId, date: today });
    if (!progress || progress.allTasksBonus) return res.status(400).json({ success: false });
    
    if (!progress.tasks.every(t => t.completed)) {
      return res.status(400).json({ success: false, message: 'Not all completed' });
    }
    ...
    progress.allTasksBonus = true;
    await progress.save();
    
    await new Transaction({ userId, type: 'bonus', amount: bonus, description: 'All daily tasks bonus' }).save();
...
app.get('/api/users/:id/streak', async (req, res) => {
  try {
    ...
    res.json({
      ...
      claimedBonuses: streak?.streakBonusesClaimed || []
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/users/:id/streak/claim/:day', async (req, res) => {
  try {
    ...
    const streak = await UserStreak.findOne({ userId });
    if (!streak || streak.currentStreak < dayNum) {
      return res.status(400).json({ success: false });
    }
    if (streak.streakBonusesClaimed?.includes(dayNum)) {
    ...
    if (!bonusConfig) return res.status(404).json({ success: false });
    
    const user = await User.findById(userId);
    user.balance += bonusConfig.bonus;
    await user.save();
    
    if (!streak.streakBonusesClaimed) streak.streakBonusesClaimed = [];
    streak.streakBonusesClaimed.push(dayNum);
    await streak.save();
    
    await new Transaction({ userId, type: 'bonus', amount: bonusConfig.bonus, description: `Streak: ${dayNum} days` }).save();
...
app.get('/api/users/:id/achievements', async (req, res) => {
  try {
    ...
    const achievements = configs.map(config => {
      const userAch = user?.achievements?.find(a => a.id === config.code);
      return {
        code: config.code, icon: config.icon, name: config.name, description: config.description,
        reward: config.reward, unlocked: !!userAch, unlockedAt: userAch?.unlockedAt
      };
    });
...
app.get('/api/leaderboard/:period', async (req, res) => {
  try {
    ...
    let leaderboard = await Leaderboard.findOne({ period, city: cityKey });
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
    
    if (!leaderboard || leaderboard.updatedAt < oneHourAgo) {
      // Обновляем лидерборд
      let dateFilter = {};
      const now = new Date();
      ...
      const stats = await Parking.aggregate([
        ...
        { $group: { _id: '$ownerId', parkingsGiven: { $sum: 1 }, totalEarned: { $sum: '$price' } } },
        { $sort: { parkingsGiven: -1 } },
        { $limit: 100 }
      ]);
      ...
      for (let i = 0; i < stats.length; i++) {
        const stat = stats[i];
        const user = await User.findById(stat._id).select('name avatar parkingsGiven');
        if (!user) continue;
        
        const level = await calculateUserLevel(user);
        rankings.push({
          userId: stat._id, name: user.name, avatar: user.avatar,
          level: level.level, levelIcon: level.icon,
          score: stat.parkingsGiven * 10 + stat.totalEarned,
          parkingsGiven: stat.parkingsGiven, rank: i + 1
        });
      }
...
// --- Seasonal Event ---
app.get('/api/seasonal-event/active', async (req, res) => {
  try {
    const now = new Date();
    const event = await SeasonalEvent.findOne({
...
app.get('/api/admin/game-settings', async (req, res) => {
  try {
    let settings = await GameSettings.findOne({ key: 'main' });
    
    if (!settings) {
      settings = new GameSettings({
        key: 'main',
        levels: [
          { level: 1, name: { en: 'Newbie', ru: 'Новичок', es: 'Novato', uk: 'Новачок' }, icon: '🚗', minPoints: 0, minParkingsGiven: 0 },
          { level: 2, name: { en: 'Driver', ru: 'Водитель', es: 'Conductor', uk: 'Водій' }, icon: '🚙', minPoints: 100, minParkingsGiven: 5 },
          { level: 3, name: { en: 'Master', ru: 'Мастер', es: 'Maestro', uk: 'Майстер' }, icon: '🏎️', minPoints: 500, minParkingsGiven: 25 },
          { level: 4, name: { en: 'Legend', ru: 'Легенда', es: 'Leyenda', uk: 'Легенда' }, icon: '👑', minPoints: 2000, minParkingsGiven: 100 }
        ],
        streakBonuses: [
          { day: 1, bonus: 5 }, { day: 3, bonus: 10 }, { day: 7, bonus: 25 },
          { day: 14, bonus: 50 }, { day: 30, bonus: 100 }
        ],
        dailyTaskReward: 10, allDailyTasksBonus: 25, pointsMultiplier: 1.0
      });
      await settings.save();
    }
    
    const tasks = await DailyTaskConfig.find();
    const achievements = await AchievementConfig.find();
    const events = await SeasonalEvent.find().sort({ createdAt: -1 });
    
    res.json({ settings, tasks, achievements, events });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/admin/game-settings', async (req, res) => {
  try {
    await GameSettings.findOneAndUpdate({ key: 'main' }, { ...req.body.settings, updatedAt: new Date() }, { upsert: true });
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ success: false });
  }
...
// Admin CRUD for tasks, achievements, events
app.post('/api/admin/daily-tasks', async (req, res) => {
  try { const task = await new DailyTaskConfig(req.body).save(); res.json({ success: true, task }); }
  catch (e) { res.status(500).json({ success: false }); }
});
app.put('/api/admin/daily-tasks/:id', async (req, res) => {
  try { const task = await DailyTaskConfig.findByIdAndUpdate(req.params.id, req.body, { new: true }); res.json({ success: true, task }); }
  catch (e) { res.status(500).json({ success: false }); }
});
app.delete('/api/admin/daily-tasks/:id', async (req, res) => {
  try { await DailyTaskConfig.findByIdAndDelete(req.params.id); res.json({ success: true }); }
  catch (e) { res.status(500).json({ success: false }); }
});

app.post('/api/admin/achievements', async (req, res) => {
  try { const ach = await new AchievementConfig(req.body).save(); res.json({ success: true, achievement: ach }); }
  catch (e) { res.status(500).json({ success: false }); }
});
app.put('/api/admin/achievements/:id', async (req, res) => {
  try { const ach = await AchievementConfig.findByIdAndUpdate(req.params.id, req.body, { new: true }); res.json({ success: true, achievement: ach }); }
  catch (e) { res.status(500).json({ success: false }); }
});
app.delete('/api/admin/achievements/:id', async (req, res) => {
  try { await AchievementConfig.findByIdAndDelete(req.params.id); res.json({ success: true }); }
  catch (e) { res.status(500).json({ success: false }); }
});

app.post('/api/admin/seasonal-events', async (req, res) => {
  try { const event = await new SeasonalEvent(req.body).save(); res.json({ success: true, event }); }
  catch (e) { res.status(500).json({ success: false }); }
});
app.put('/api/admin/seasonal-events/:id', async (req, res) => {
  try { const event = await SeasonalEvent.findByIdAndUpdate(req.params.id, req.body, { new: true }); res.json({ success: true, event }); }
  catch (e) { res.status(500).json({ success: false }); }
});
app.delete('/api/admin/seasonal-events/:id', async (req, res) => {
  try { await SeasonalEvent.findByIdAndDelete(req.params.id); res.json({ success: true }); }
  catch (e) { res.status(500).json({ success: false }); }
});
